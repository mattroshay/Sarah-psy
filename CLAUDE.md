...
- **Calendly is two URLs, not one.** `NEXT_PUBLIC_CALENDLY_DISCOVERY_URL` (free 15-min call) and `NEXT_PUBLIC_CALENDLY_SESSION_URL` (paid session) are passed as separate props (`discoveryCalendlyUrl`, `sessionCalendlyUrl`) into `ContactPage`, where they render as two side-by-side inline iframes. `CalendlyButton` opens the discovery URL as a popup overlay (added in PRs #7–#8). When either env var is missing, the corresponding iframe renders the dictionary `contact.calendlyFallback` message ("Booking link not configured"); when the discovery var is missing for non-contact CTAs, they fall back to a locale-aware `/${locale}/contact` link instead of breaking. The old single-var `NEXT_PUBLIC_CALENDLY_URL` was removed in commit `eee2f3e` — do not reintroduce it.
- **Testimonials are live** on the home page with three real reviews (Lucy, an anonymous patient, and Ocindy), each in EN and FR. Every item in the `home.testimonials` array carries a short `quote` (shown on the card) and a `fullQuote` (shown in a pop-up). The "Read full review" button opens `components/ui/TestimonialModal.tsx` — a `'use client'` component built on the native `<dialog>` element (`showModal()` gives focus-trapping, Esc-to-close and a `::backdrop` for free; backdrop-click also dismisses). Items also support an optional `readMoreUrl` — Lucy's points at her Google Business Profile review and renders a small secondary "View on Google" outbound link beside the modal trigger. Labels are separate top-level dictionary keys (`home.testimonialsReadMore`, `home.testimonialsViewOnGoogle`, `home.testimonialsCloseLabel`) so they're locale-aware. To add another testimonial, append to the array in both `lib/i18n/en.ts` and `lib/i18n/fr.ts` (TS strict will yell if locales drift — `fullQuote` is required). A small disclaimer (`home.testimonialsDisclaimer`) sits under the grid explaining the translation provenance and consent/anonymisation policy.
