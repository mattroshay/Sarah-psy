...
- **Testimonials are live** on the home page with two real reviews (Lucy + an anonymous patient), each in EN and FR. Items in the `home.testimonials` array support an optional `readMoreUrl` — Lucy's points at her Google Business Profile review and renders a small "Read full review" outbound link below the attribution. The link label is a separate top-level dictionary key (`home.testimonialsReadMore`) so it's locale-aware. To add another testimonial, append to the array in both `lib/i18n/en.ts` and `lib/i18n/fr.ts` (TS strict will yell if locales drift). A small disclaimer (`home.testimonialsDisclaimer`) sits under the grid explaining the translation provenance and consent/anonymisation policy.
